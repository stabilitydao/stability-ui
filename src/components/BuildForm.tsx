...
import { account, platformData, userBalance, lastTx } from "../state/StabilityStore";
...
export function BuildForm(props: Props) {
    ...
    async function approve() {
        if (p) {
            ...
            const transaction = await _publicClient.waitForTransactionReceipt(r)
            if (transaction.status === "success") {
                setAllowance(await readContract(
                    _publicClient,
                    {
                        address: p.buildingPayPerVaultToken,
                        abi: ERC20Abi,
    ...
    const payPerVaultToken = p?.buildingPayPerVaultToken ? getTokenData(p.buildingPayPerVaultToken) : undefined
        ...
        <div className="build-form">
            {buildResult !== true &&
                <>
                    ...
                    <div className="group">
                        ...
                        <span>{props.buildingPrice && payPerVaultToken ? `${formatUnits(props.buildingPrice, payPerVaultToken.decimals)} ${payPerVaultToken.symbol}` : '-'}</span>
                    </div>
                    <div className="group">
                        <span className="group-label">Your balance</span>
                        <span>{balance?.buildingPayPerVaultTokenBalance && payPerVaultToken ? `${formatUnits(balance.buildingPayPerVaultTokenBalance, payPerVaultToken.decimals)} ${payPerVaultToken.symbol}` : '-'}</span>
                    </div>
                    ...
                    {props.vaultType === 'Rewarding' && 
                        <>
                            <div className="group">
                                ...
                                <span>{getTokenData(props.initParams.initVaultAddresses[0])?.symbol}</span>
                            </div>

                            <div>Initial boost rewards</div>
                            <div>
                                {[...new Set([props.initParams.initVaultAddresses[0], ...props.defaultBoostTokens])].map(addresss => {

                                    return (
                                        <span>{getTokenData(addresss)?.symbol}</span>
                                    )
                                })}
                            </div>
            ...
            {buildResult === undefined &&
                <div className="action">
                    {needCheckAllowance && allowance !== undefined && allowance < props.buildingPrice ? (
                        <button className="btn btn-primary" onClick={approve}>Approve factory to spend {payPerVaultToken?.symbol}</button>
                    ) : (
