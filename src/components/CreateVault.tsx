...
import { platformData, publicClient, lastTx } from '../state/StabilityStore'
...
export default function CreateVault() {
    ...
    const [allowedBBTokenVaults, setAllowedBBTokenVaults] = useState<AllowedBBTokenVaults|undefined>()
    const [minInitialBoostPerDay, setMinInitialBoostPerDay] = useState<bigint|undefined>()
    const [minInitialBoostDuration, setMinInitialBoostDuration] = useState<bigint|undefined>()
    ...
    useEffect(() => {
        async function read() {
            if (_publicClient && p) {
                ...
                if (Array.isArray(wtb)) {
                    for (let i = 0; i < wtb[1].length; i++) {
                        ...
                        for (let k = 0; k < paramsLen; ++k) {
                            initParams.initVaultAddresses[k] = wtb[4][Number(wtb[3][i][0]) + k];
                        }
                        ...
                        for (let k = 0; k < paramsLen; ++k) {
                            initParams.initStrategyAddresses[k] = wtb[6][Number(wtb[3][i][4]) + k];
                        }
                        paramsLen = wtb[3][i][7] - wtb[3][i][6];
                        for (let k = 0; k < paramsLen; ++k) {
                            initParams.initStrategyNums[k] = BigInt(wtb[7][Number(wtb[3][i][6]) + k]);
                        }
                        paramsLen = wtb[3][i][9] - wtb[3][i][8];
                        for (let k = 0; k < paramsLen; ++k) {
                            initParams.initStrategyTicks[k] = wtb[8][Number(wtb[3][i][8]) + k];
                        }
                ...
                setMinInitialBoostPerDay(
                    await _publicClient.readContract({
                        address: platform,
                        functionName: 'minInitialBoostPerDay',
                        abi: PlatformAbi
                    }) as bigint
                )
                setMinInitialBoostDuration(
                    await _publicClient.readContract({
                        address: platform,
                ...
                console.log(await _publicClient.readContract({
                    address: platform,
                    functionName: 'defaultBoostRewardTokens',
                    abi: PlatformAbi
                }) as string[])
    ...
    const compoundingVaultsForBuilding = buildVariants.filter(v => v.vaultType === 'Compounding').length

    return (
        <div>
            <h2>Create compounding vault</h2>
            {compoundingVaultsForBuilding > 0 ? 
                <table>
                <thead>
                    <tr>
                        <td></td>
            ...
            <div className='rewarding-data'>
                {allowedBBTokenVaults !== undefined && 
                    <div>
                        For allowed buy-back tokens, the following number of rewarding vaults can be created:
                        <div>
                            {Object.keys(allowedBBTokenVaults).map(t => {
                                ...
                                return (
                                    <div key={t} className='allowed-bb-row'>
                                        <span>
                                            {token ? <img src={token.logoURI} alt={token.name} className='token-logo' /> : ''}
                                            {token?.symbol || t}
                                        </span>
                ...
                {minInitialBoostPerDay !== undefined &&
                    <div>
                        Minimal initial boost value per day: ${formatUnits(minInitialBoostPerDay, 18)}
                    </div>
                }
                <div>
                    Buy-back rewards vesting duration: 7 days
                </div>
                <div>
                    Boost rewards vesting duration: 30 days
                </div>
                {/* {minInitialBoostDuration !== undefined &&
                    <div>
                        Minimal initial boost duration: {(minInitialBoostDuration / BigInt(86400)).toString()} days
                    </div>
                } */}
                <br />
            </div>
                    ...
                    {buildVariants.map((v, i) => {
                        ...
                        return (
                            <tr key={v.strategyDesc + v.vaultType + v.strategyId}>
                                ...
                                <td>{v.vaultType}</td>
                                <td>{getTokenData(buildVariants[i].initParams.initVaultAddresses[0])?.symbol}</td>
                                <td>{v.strategyId}</td>
                                <td>{v.strategyDesc}</td>
                            </tr>
                        )
            ...
            <div className='rewarding-data'>
                ...
                {allowedBBTokenVaults !== undefined && 
                    <div>
                        For allowed buy-back tokens, the following number of rewarding managed vaults can be created:
                        <div>
                            {Object.keys(allowedBBTokenVaults).map(t => {
                                const token = getTokenData(t)
            ...
            <table>
                ...
                <tbody>
                    {buildVariants.map((v, i) => {
                        if (v.vaultType !== 'Rewarding Managed') {
                            return
                        }
            ...
            {p && buildIndex !== undefined && (
                <div
                    className='overlay'
                    onClick={() => {setBuildIndex(undefined)}}
                >
                    <div
                        className='modal'
                    ...
                    >
                        ...
                        <BuildForm
                            ...
                            buildingPrice={p.buildingPrices[buildVariants[buildIndex].vaultType]}
