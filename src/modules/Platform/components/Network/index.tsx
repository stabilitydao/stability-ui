          ...
          .filter(
            (machineId: string) =>
              new Date().getTime() / 1000 -
                Number($apiData.network.nodes[machineId]?.lastSeen) <
              86400 * 2
          )
          .map((machineId) => {
            ...
            const isOnline = !!(
              node?.lastSeen &&
              new Date().getTime() / 1000 - node.lastSeen < 180
            );
            // @ts-ignore
            return (
              <a
              ...
              >
                {node.seedNode ? (
                  ...
                  <div className="flex items-center text-[20px] py-2 px-5 bg-transparent rounded-t-2xl justify-between">
                    <span className="text-gray-400">
                      {node.hostname ?? shortMachineId(machineId)}
                    </span>
                    <span>
                      {isOnline ? (
                        <div className="inline-flex w-[12px] h-[12px] rounded-full bg-green-600"></div>
                ...
                {isOnline ? (
                  ...
                  <div className="flex flex-col text-[14px] px-6 pt-4">
                    <span>
                      Last seen:{" "}
                      {formatLifeTime(
                        new Date().getTime() / 1000 - Number(node?.lastSeen)
                      )}{" "}
                      ago
                    </span>
                  </div>
                ...
                <div className="flex flex-col p-6 pt-4">
                  <div className="flex-col text-[14px]">
                    {node.services
                      .filter((s) => !s.name.match(/network|api/i))
                      .map((service) => (
                        <div
                          key={service.name}
                        ...
                        >
                          ...
                          {service.id && (
                            <span className="text-[12px] text-accent-300">
                              {getShortAddress(service.id, 6, 4)}
                            </span>
                          )}
                        </div>
                      ))}
