 ...
 *
 * ```
 * const strategyInfo = getStrategyInfo("GQMF");
 * ```
 *
 ...
 * @returns {string} name - Full name of strategy (e.g., "Gamma QuickSwap Farm")
 * @returns {string} shortName - Short name or identifier for strategy (e.g., "GQF")
 * @returns {Array} protocols - List of protocols involved in the strategy (e.g., ["Gamma", "QuickSwap"])
 * @returns {Array} features - List of features for the strategy, such as farming options
 * @returns {string} color - Primary color associated with the strategy
 * @returns {string} bgColor - Background color for the strategy
 * @returns {Array} baseStrategies - List of base strategies (e.g., "Farming", "Liquidity providing")
